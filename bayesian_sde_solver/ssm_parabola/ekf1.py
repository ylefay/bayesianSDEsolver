...
def solver(key, init, delta, drift, diffusion, h, N, sqrt=False, prior=None, noise=None):
    """
    ...
    filtered = _solver(init, drift, diffusion, delta, h, N, sqrt, EKF0=False, prior=prior, noise=noise)
    m, P = filtered
    if key is not None:
