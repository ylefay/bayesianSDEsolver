...
def solver(key, init, vector_field, h, N, sqrt=True, prior=None, noise=None):
    """
    ...
    filtered = _solver(init, vector_field, h, N, sqrt, EKF0=False, prior=prior, noise=noise, n_states=n_states)
    m, P = filtered
    if key is not None:
        sqrtP = P if sqrt else jnp.real(linalg.sqrtm(P))
        last_sample = m + sqrtP @ jax.random.multivariate_normal(key, jnp.zeros((n_states * dim,)), jnp.eye(n_states * dim))
