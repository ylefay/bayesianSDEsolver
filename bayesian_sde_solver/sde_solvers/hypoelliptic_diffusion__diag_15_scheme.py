...
def solver(key, init, drift, sigma, h, N):
    ...
    def body(x, key):
        ...
        epsilon_k = jax.random.multivariate_normal(bm_key, jnp.zeros(dim),
                                                   jnp.block([[jnp.reshape(
                                                       partialua(x) @ GammaGammaT(x) @ partialua(x).T * h ** 3 / 3,
                                                       (1, 1)), jnp.reshape(partialua(x) @ GammaGammaT(x) * h ** 2 / 2,
                                                                            (1, dim - 1))],
                                                              [jnp.reshape(partialua(x) @ GammaGammaT(x) * h ** 2 / 2,
                                                                           (dim - 1, 1)), GammaGammaT(x) * h]])
                                                   )  # up to leading order...
        out = x + h * drift(x) + h ** 2 / 2 * partialuvdrift(x) @ drift(x) + h ** 2 / 4 * laplaceweighted(sigma, drift,
                                                                                                          x) + epsilon_k
        return out, out
...
def exact_solver(key, init, drift, sigma, h, N):
    ...
    def body(x, key):
        key_k = key
        bm_key = jax.random.split(key_k, 1)
        zeta_eta_k = jax.random.multivariate_normal(bm_key, jnp.zeros(2 * dim - 2), jnp.block(
            [[jnp.eye(dim - 1) * h ** 3 / 3, jnp.eye(dim - 1) * h ** 2 / 2],
        ...
        second_term_U = Gamma(x) @ eta_k + partialuA(x) @ Gamma(x) @ zeta_k + \
                        ...
                        partialuGamma_diag(x) @ A(x) @ (h * eta_k - zeta_k) + 0.5 * laplacegamma(x) @ (
                                    h * eta_k - zeta_k) + \
                        0.5 * (partialuGamma_diag(x) @ partialuGamma_diag(x) @ Gamma(x) + laplacegamma(x) @ (
                    1 / 3 * eta_k @ eta_k.T - h * jnp.ones((dim - 1,)))) @ eta_k
