...
def experiment(delta, N, M, fine):
    ...
    def sde_solver(
        ...
        def body(x, inp):
            ...
            _, euler_path = euler_maruyama_pathwise(total_inc, init=x1, drift=drift_shifted_ito,
                                                    sigma=sigma_shifted_ito,
                                                    h=delta, N=1)
            dt = delta / fine
            _, fine_euler_path = euler_maruyama_pathwise(incs, init=x2, drift=drift_shifted_ito,
                                                    sigma=sigma_shifted_ito,
                                                    h=dt, N=fine)
            next_x = euler_path[-1]
            next_x2 = fine_euler_path[-1]
