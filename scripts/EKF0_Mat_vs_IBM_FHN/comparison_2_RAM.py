...
solver_name = "EKF0"
problem_name = "FHN"
prefix = f"{solver_name}_{problem_name}_SAME_PRIOR"
folder = "./"
...
def experiment(delta, N, M, fine):
    ...
    prior_ioup = IOUP_transition_function(theta=0., sigma=1.0, dt=delta / M, q=order, dim=x0.shape[0])
    prior_mat = matern_transition_function(k=order, magnitude=1.0, length=1.0, dt=delta / M, dim=x0.shape[0])
