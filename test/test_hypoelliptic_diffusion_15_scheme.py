...
def test_fitzhugh_nagumo():
    ...
    def theoretical_mean_up_to_order_2(k):
        t = k * h
        return x0 + t * jnp.array([1 / eps * (x1 - x1 ** 3 - x2 + s) + t / 2 * 1 / eps * (
                    1 / eps * (1 - 3 * x1 ** 2) * (x1 - x1 ** 3 - x2 + s) - (gamma * x1 - x2 - alpha)),
                                   gamma * x1 - x2 + alpha + t / 2 * (
                                               gamma / eps * (x1 - x1 ** 3 - x2 + s) - (gamma * x1 - x2 + alpha))])

    def theoretical_variance_up_to_order3(k):
        t = k * h
        return sig ** 2 * jnp.array([[1 / 3 * t ** 3 * 1 / eps ** 2, - 1 / 2 * t ** 2 * 1 / eps],
                                     [- 1 / 2 * t ** 2 * 1 / eps, t - t ** 2]])
